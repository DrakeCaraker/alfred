...
lint:
	@if command -v shellcheck >/dev/null 2>&1; then \
		...
		shellcheck -S warning .claude/hooks/*.sh .githooks/pre-push .githooks/pre-commit scripts/*.sh 2>&1; \
	...
	@if command -v ruff >/dev/null 2>&1; then \
		echo "Running ruff..."; \
		ruff check collective/ 2>&1; \
		echo "ruff: passed"; \
	elif command -v python3 >/dev/null 2>&1 && python3 -m ruff --version >/dev/null 2>&1; then \
		echo "Running ruff (via python3 -m)..."; \
		python3 -m ruff check collective/ 2>&1; \
		echo "ruff: passed"; \
	else \
